...
Erfordert die Einbindung des [`cFos PowerBrain`](meters#powerbrain) Strommessgerätes.
...
Untersützt die Wallboxen go-eCharger Home, go-eCharger PRO.

:::note
Mindestens Firmware 040.0 oder neuer benötigt.
:::
...
:::note
Mindestens Firmware 052.1 oder neuer benötigt.
:::
...
<Config part="chargers" file="innogy-ebox" />
...
<Config part="chargers" file="amtron" />
...
<Config part="chargers" file="senec-pro" />

<SponsorshipRequired />

### SENEC.Wallbox pro s

<Config part="chargers" file="senec-pro-s" />
...
Demgegenüber dient der *Static-Switch* Modus zum einfachen Schalten von Verbrauchern, wie Wärmepumpen, Klimaanlagen etc.
...
Ist der Wert größer oder gleich 0, ist der *Charger Modus* aktiviert. Der eingestellte Wert dient als Schwellwert in Watt.
...
Stetzt man `standbypower` auf einen negativen Wert arbeitet die schaltbare Steckdose im *Static-Switch* Modus. 
...
- Starte `evcc eebus-cert` (falls noch nicht geschehen)
- Füge die Ausgabe von `evcc eebus-cert` in die `evcc.yaml` Konfigurationsdatei ein.
- Öffne die Web Oberfläche der Wallbox, um die SKI-Gerätekennung (Identifcation Number) zu finden.   
Beim Porsche Mobile Charger Connect kann man die Information im Menü "Connections", Untermenü "Energy Manager" finden.
- Füge den Charger hinzu
  <Config part="chargers" file="eebus" />
- Starte `evcc`
- Auf der Web Oberfläche der Wallbox, üblicherweise auf der gleichen Seite wie zuvor, sollte nun `evcc` erscheinen inklusive einer Option, um die Wallbox mit `evcc` zu verbinden (pairen).   
